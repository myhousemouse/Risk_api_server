...
**핵심 규칙:**
1. **반드시 위 사업 내용("{business_description}")을 자세히 읽고, 그 사업에만 해당하는 구체적인 질문을 만드세요**
2. 일반적인 질문 금지! 모든 비즈니스에 적용되는 뻔한 질문은 절대 안 됩니다
3. 정확히 2개 또는 3개의 질문만 생성하세요
4. 초보 창업자가 이해할 수 있는 쉬운 말로 질문하세요 (전문 용어 사용 금지)
5. 측정 가능하거나 구체적인 숫자/사실을 묻는 질문을 만드세요

**출력 형식:**
Q1: [질문] | [text/number/choice] | [선택지]

**예시 - "{business_description}"라는 사업이라면:**
- ✅ 좋은 질문: "{business_description}"에서 제공하는 핵심 기능/서비스는 무엇이며, 경쟁사는 이것을 제공하나요?
- ✅ 좋은 질문: "{business_description}"의 주요 고객은 하루에/한 달에 몇 번 이용할 것 같나요?
- ❌ 나쁜 질문: 경쟁사 분석을 했나요? (너무 일반적)
- ❌ 나쁜 질문: 마케팅 계획이 있나요? (너무 일반적)
"""
    ...
    def _get_industry_specific_context(self, business_description: str, method: AnalysisMethod) -> str:
        ...
        if any(kw in description_lower for kw in ["앱", "소프트웨어", "플랫폼", "IT", "개발", "스타트업", "웹", "모바일"]):
            return f"""
**[IT/앱/플랫폼 사업 전용 질문 가이드]**
"{business_description}" 사업에 대해 다음 중 가장 중요한 것들을 물어보세요:
- 개발자가 있나요? 몇 명이며, 개발 기간은 얼마나 걸리나요?
- 앱/서비스를 출시하면 처음 한 달에 사용자가 몇 명 정도 올까요?
- 비슷한 앱/서비스가 이미 있나요? 그것과 뭐가 다른가요?
- 돈을 어떻게 벌 건가요? (광고? 유료 구독? 수수료?)
- 서버/클라우드 비용은 한 달에 얼마나 드나요?
"""
        elif any(kw in description_lower for kw in ["교육", "학습", "강의", "학원", "에듀테크", "교육용"]):
            return f"""
**[교육/학습 사업 전용 질문 가이드]**
"{business_description}" 사업에 대해 다음 중 가장 중요한 것들을 물어보세요:
- 강사나 선생님은 확보했나요? 몇 명이며, 어떻게 구할 건가요?
- 학생/수강생을 어떻게 모을 건가요?
- 수강료는 얼마이며, 한 달에 몇 명의 학생이 필요한가요?
- 학습 효과를 어떻게 증명할 건가요? (성적 향상? 자격증 취득률?)
- 경쟁 학원/강의와 비교해서 우리만의 특별한 점은 뭔가요?
"""
        elif any(kw in description_lower for kw in ["제조", "생산", "공장", "설비", "제품 생산", "양산"]):
            return f"""
**[제조/생산 사업 전용 질문 가이드]**
"{business_description}" 사업에 대해 다음 중 가장 중요한 것들을 물어보세요:
- 생산 설비(기계)는 이미 있나요? 없으면 얼마에 살 수 있나요?
- 하루에/한 달에 몇 개를 만들 수 있나요?
- 불량품(결함 있는 제품)은 100개 중 몇 개 정도 나올까요?
- 원자재(재료)는 어디서 구하며, 공급이 안정적인가요?
- 납품 기한을 못 맞추면 어떻게 되나요?
"""
        elif any(kw in description_lower for kw in ["외식", "음식점", "카페", "레스토랑", "서비스", "매장", "가게"]):
            return f"""
**[서비스/외식 사업 전용 질문 가이드]**
"{business_description}" 사업에 대해 다음 중 가장 중요한 것들을 물어보세요:
- 가게 위치는 어디이며, 하루에 지나가는 사람이 몇 명 정도인가요?
- 음식/상품 재료는 어디서 사고, 원가는 매출의 몇 %인가요?
- 위생 관리는 어떻게 할 건가요? (식약처 검사 대비)
- 처음 온 손님이 다시 올 확률이 얼마나 될까요?
- 비슷한 가게가 근처에 몇 개나 있나요?
"""
        elif any(kw in description_lower for kw in ["마케팅", "광고", "브랜드", "홍보", "프로모션"]):
            return f"""
**[마케팅/광고 사업 전용 질문 가이드]**
"{business_description}" 사업에 대해 다음 중 가장 중요한 것들을 물어보세요:
- 타겟 고객은 정확히 누구인가요? (나이, 성별, 직업 등)
- 광고 예산은 얼마이며, 어떤 채널에 쓸 건가요? (SNS? 유튜브? 네이버?)
- 광고 효과를 어떻게 측정할 건가요?
- 경쟁사는 어떤 마케팅을 하고 있나요?
- 브랜드를 사람들이 기억하게 만들 특별한 방법이 있나요?
"""
        else:
            return f"""
**[일반 비즈니스 질문 가이드]**
"{business_description}" 사업에 대해 다음을 물어보세요:
- 고객이 정확히 누구이며, 이들은 왜 우리 제품/서비스를 사야 하나요?
- 어떻게 돈을 벌 건가요? 한 달 수입 목표는 얼마인가요?
- 경쟁자가 있나요? 우리가 더 나은 점은 뭔가요?
- 지금 가진 돈으로 몇 개월 버틸 수 있나요?
"""
